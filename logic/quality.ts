...
export function calcQuality(
  ...
  // ✅ PASSO 3: googStarsMes/tripStarsMes calculados separadamente abaixo
  ...
  // ✅ PASSO 3: Nota Mês = todas as semanas fechadas do mês da data final (independente do período)
  const semanasUltimoMes = semanasFecharadas(ultimoMes.ano, ultimoMes.mes0);
  googS.filter((r) =>
    r.mes === ultimoMesStr && r.ano === ultimoMesAno && matchLoja(r.loja) &&
    semanasUltimoMes.some((s) => inSemana(r.data, s.seg, s.dom))
  ).forEach((r) => {
    googStarsMes[0]+=r.n1||0; googStarsMes[1]+=r.n2||0;
    googStarsMes[2]+=r.n3||0; googStarsMes[3]+=r.n4||0; googStarsMes[4]+=r.n5||0;
  });
  tripS.filter((r) =>
    r.mes === ultimoMesStr && r.ano === ultimoMesAno && matchLoja(r.loja) &&
    semanasUltimoMes.some((s) => inSemana(r.data, s.seg, s.dom))
  ).forEach((r) => {
    tripStarsMes[0]+=r.n1||0; tripStarsMes[1]+=r.n2||0;
    tripStarsMes[2]+=r.n3||0; tripStarsMes[3]+=r.n4||0; tripStarsMes[4]+=r.n5||0;
  });
  ...
  meses.forEach(({ ano, mes0 }) => {
    ...
    if (!mesAberto) {
      ...
      // ✅ PASSO 1: filtrar tentativas por semanas dentro do período
      const semanasDoMes = semsF.filter((s) => s.dom >= ini && s.seg <= fim);
      npsS.filter((r) =>
        r.mes === ms && r.ano === as && matchLoja(r.loja) &&
        semanasDoMes.some((s) => inSemana(r.data, s.seg, s.dom))
      ).forEach((r) => { tentAcum += r.tent || 0; });
      fM(npsM).forEach((r) => {
        if (!porLoja[r.loja]) porLoja[r.loja] = { npsVals: [] };
        if (r.nps > 0) porLoja[r.loja].npsVals.push(r.nps);
      });
      // starsPeriodo: todas as semanas do mês fechado
      const fSMes = (arr: any[]) => arr.filter((r) => r.mes === ms && r.ano === as && matchLoja(r.loja));
      fSMes(googS).forEach((r) => { googStarsPeriodo[0]+=r.n1||0; googStarsPeriodo[1]+=r.n2||0; googStarsPeriodo[2]+=r.n3||0; googStarsPeriodo[3]+=r.n4||0; googStarsPeriodo[4]+=r.n5||0; });
      fSMes(tripS).forEach((r) => { tripStarsPeriodo[0]+=r.n1||0; tripStarsPeriodo[1]+=r.n2||0; tripStarsPeriodo[2]+=r.n3||0; tripStarsPeriodo[3]+=r.n4||0; tripStarsPeriodo[4]+=r.n5||0; });
    } else {
      ...
      fSGT(googS).forEach((r) => { googStarsPeriodo[0]+=r.n1||0; googStarsPeriodo[1]+=r.n2||0; googStarsPeriodo[2]+=r.n3||0; googStarsPeriodo[3]+=r.n4||0; googStarsPeriodo[4]+=r.n5||0; });
      fSGT(tripS).forEach((r) => { tripStarsPeriodo[0]+=r.n1||0; tripStarsPeriodo[1]+=r.n2||0; tripStarsPeriodo[2]+=r.n3||0; tripStarsPeriodo[3]+=r.n4||0; tripStarsPeriodo[4]+=r.n5||0; });
      fS(npsS).forEach((r) => {
        if (!porLoja[r.loja]) porLoja[r.loja] = { npsVals: [] };
        if (r.nps > 0) porLoja[r.loja].npsVals.push(r.nps);
      });
  ...
  // ✅ PASSO 1: passantes usando fim em vez de ultimaSemanaNPS
  const fluxoFiltrado = fluxoBase.filter((r) => r.date >= ini && r.date <= fim && matchLoja(r.loja));
